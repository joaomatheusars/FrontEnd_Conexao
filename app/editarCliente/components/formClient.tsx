...
interface FormClientProp {
  client: any;
  id: string;
  token: string

}
const FormClient = ({ client, id, token }: FormClientProp) => {
  ...
  const registerUser = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("http://localhost:8000/clientes/updateInfoClient?" + new URLSearchParams(data), {
        method: "POST",
    ...
    <>
      ...
      <div className="flex h-full flex-col items-center justify-center">
        <div className="container flex flex-col gap-8 rounded-md bg-table p-8">
          <h1 className="text-5xl font-extrabold text-white">Editar Cliente</h1>
                  ...
                  <div className="flex flex-wrap gap-8">
                    <div className="flex items-center gap-4">
                      ...
                      <IMaskInput
                        ...
                        placeholder={client.cep}
                        // onChange={(e: any) => handleGetCEP(e.target.value)}
                      />
                      <button
                        onClick={handleGetCEP}
                        type="button"
                        className="flex gap-3 rounded-full bg-blue-500 p-2 font-bold"
                      >
                        <Search />
                        Buscar
                      </button>
